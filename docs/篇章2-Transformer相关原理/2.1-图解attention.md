...
1. 假设序列输入是一个句子，这个句子可以由$n$个词表示: $$sentence = \{w_1, w_2,...,w_n\}$$ ![formula](https://render.githubusercontent.com/render/math?math=sentence = \{w_1, w_2,...,w_n\})。
3. RNN首先将句子中的每一个词映射成为一个向量得到一个向量序列：$X = \{x_1, x_2,...,x_n\}$，每个单词映射得到的向量通常又叫做：word embedding。
4. 然后在处理第$t \in [1,n]$个时间步的序列输入$x_t$时，RNN网络的输入和输出可以表示为：$h_{t} = RNN(x_t, h_{t-1})$
